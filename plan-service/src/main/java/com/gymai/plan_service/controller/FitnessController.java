...
// public ResponseEntity<CompleteFitnessPlan> generateCompletePlan(@RequestBody
// User user) {
...
// workoutPlanService.generateCustomWorkoutPlan(savedUser);
...
// "Complete fitness plan for %s - Goal: %s | Daily Calories: %.0f | Workout
// Days: %d/week",
// savedUser.getName(), savedUser.getGoal(), dietPlan.getDailyCalorieTarget(),
// (int) workoutPlan.getWeeklyPlan().stream().filter(day ->
// !day.isRestDay()).count()));

// log.info("Successfully generated and persisted complete plan for userId={}",
// savedUser.getUserId());
// return ResponseEntity.ok(completePlan);

// } catch (Exception e) {
// log.error("Error generating complete fitness plan for user: {}",
// user.getEmail(), e);
...
// User savedUser = userRepository.save(user);
// DietPlan dietPlan = dietPlanService.generateCustomDietPlan(savedUser);
// log.info("Diet plan generated and persisted for userId={}",
// savedUser.getUserId());
...
// public ResponseEntity<WorkoutPlan> generateWorkoutPlan(@RequestBody User
// user) {
...
// WorkoutPlan workoutPlan =
// workoutPlanService.generateCustomWorkoutPlan(savedUser);
// log.info("Workout plan generated and persisted for userId={}",
// savedUser.getUserId());
...
// public ResponseEntity<UserPlansResponse> getUserPlans(@PathVariable Long
// userId) {
...
// if (dietPlan == null) {
// log.info("No existing diet plan found for userId={}, generating new one",
// userId);
// dietPlan = dietPlanService.generateCustomDietPlan(user);
// }

// if (workoutPlan == null) {
// log.info("No existing workout plan found for userId={}, generating new one",
// userId);
...
// WorkoutPlan workoutPlan =
// workoutPlanService.regenerateWorkoutPlan(savedUser);
...
// completePlan.setGeneratedDate(LocalDate.now());
// completePlan.setSummary(String.format(
// "Updated fitness plan for %s - Goal: %s | Daily Calories: %.0f | Workout
// Days: %d/week",
// savedUser.getName(), savedUser.getGoal(), dietPlan.getDailyCalorieTarget(),
// (int) workoutPlan.getWeeklyPlan().stream().filter(day ->
// !day.isRestDay()).count()));
...
// } catch (Exception e) {
// log.error("Error updating user and regenerating plans for userId={}", userId,
// e);
// return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(null);
// }
// }
...
// public ResponseEntity<WorkoutPlan> regenerateWorkoutPlan(@PathVariable Long
// userId) {
...
// WorkoutPlan workoutPlan =
// workoutPlanService.regenerateWorkoutPlan(userOpt.get());
...
// public ResponseEntity<NutritionAnalysis> getNutritionAnalysis(@PathVariable
// Long userId) {
...
// return (10 * user.getWeight()) + (6.25 * user.getHeight()) - (5 *
// user.getAge()) + 5;
// } else {
// return (10 * user.getWeight()) + (6.25 * user.getHeight()) - (5 *
// user.getAge()) - 161;
